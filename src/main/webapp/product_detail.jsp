    ...
    if (pidStr != null && backUrl != null) {
        try {
            ...
            product = ProductService.getInstance().getProductById(productId);
          	backUrl = backUrl.equals("product_list") ? 
          					"/Suwazon_zuichan/product_list" : 
          					"/Suwazon_zuichan/cart";        
          	} catch (Exception e) {
...
<div class="product-container">
    ...
    <% if (product != null) { %>
        ...
        <div class="image-box">
            <img src="<%=product.getImageDirectory()%>" alt="商品画像" width="150" height="100">
        </div>

        <div class="price"><%= product.getPrice() %>円</div>

        <form action="/Suwazon_zuichan/cart" method="post">
            <input type="hidden" name="product_id" value="<%=product.getProductId()%>">
            <input type="hidden" name="action" value="addProduct">
            <input type="submit" class="cart-button" value="カートに入れる">
        </form>
